...
// --- ДОДАТИ ЦЕЙ КОМПОНЕНТ ---
function NFTCard({ id, contract, nftDropAddress, tokenContract, claimedCount, currentAllowance, ACTIVE_MINT_PRICE, formatPriceLabel, getImageUrl }: any) {
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900 font-sans overflow-x-hidden relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      {activeTab === "vault" && (
        <div className="space-y-12 sm:space-y-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[claimedCount > 1 ? claimedCount - 2 : 0, claimedCount > 0 ? (claimedCount > 1 ? claimedCount - 1 : 1) : 1, claimedCount, claimedCount + 1].map((id) => (
               <NFTCard key={id} id={id} contract={nftContract} nftDropAddress={nftDropAddress} tokenContract={tokenContract} claimedCount={claimedCount} currentAllowance={currentAllowance} ACTIVE_MINT_PRICE={ACTIVE_MINT_PRICE} formatPriceLabel={formatPriceLabel} getImageUrl={getImageUrl} />
            ))}
          </div>
      ...
      {activeTab === "market" && (
        <div className="bg-white border border-slate-200 rounded-[24px] p-8 max-w-3xl mx-auto shadow-sm">
          ...
          <div className="space-y-4 text-center text-slate-400">Маркетплейс розділ активний.</div>
        </div>
      )}
